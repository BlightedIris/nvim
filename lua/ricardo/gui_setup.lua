...
-- Default layout: neo-tree (left) | editor over terminal (middle) | AI chat
-- (right). Invoked by the session logic (lua/plugins/mini-sessions.lua) on
-- boot when there is no session to restore, and by <leader>sc to reset to it;
-- it assumes a blank slate, so callers wipe the current layout first.
M.open_default_ui = function()
